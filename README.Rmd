...
Arrange by column X3 (the chromosome column)

```{r}
maize <- arrange(as.tibble(cbind(snps_f, maize_tgen[,-1])), as.numeric(Chromosome), as.numeric(Position))
teos <- arrange(as.tibble(cbind(snps_f, teos_tgen[,-1])), as.numeric(Chromosome), as.numeric(Position))
```
...
for (i in 1:nrow(test)) {
    if(i == 1 || test[i,1] != test[i-1,1]) {
        write_tsv(test[i,], paste("sorted_data/", test[i,1],"_testfile.txt", sep = ""))
    } else {
        write_tsv(test[i,], paste("sorted_data/", test[i,1],"_testfile.txt", sep = ""), append = T)
    }
}
```
