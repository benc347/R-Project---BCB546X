...
first graph - 
need list of snps with lavrovs definition
...
headed_genotypes <- as.tibble(lapply(headed_genotypes, function(x) {
  gsub("\\?\\/\\?", "NA", x) 
}))
grouped_genotypes <- arrange(headed_genotypes, Group)
```
...
Create a tibble with two columns, Groups and SNPs
For loop
  first for
...
Mutate and add a new column, Phenotype. Nested ifelse statements assign either Homo or Hetero depending on SNP. Arrange by either group or species.

```{r}
gathered_genotypes <- gather(headed_genotypes[,-1], SNP_info, values, -JG_OTU, -Group) %>%
...
graph_2_group <- arrange(gathered_genotypes, Group)
graph_2_species <- arrange(gathered_genotypes, JG_OTU)
```
...
ggplot(data = graph_2_species) + geom_bar(mapping = aes(x = JG_OTU, fill = homo_hetero), position = "fill")
```

```{r}
p <- ggplot(data = maize_asc, mapping = aes(x=SNP_ID, y=as.numeric(Position))) + geom_point(size=1, color="grey")
